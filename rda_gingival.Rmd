...
rmarkdown::render("/Users/natasha/Documents/Research/relman-lab/docker_access/sea_otter/vPaper/gingival_community_profiles/rda_gingival.Rmd", clean=TRUE, output_format="html_document")

In this script we will investigate variation in the bacterial community composition and structure of the gingival microbiota of wild sea otters. 
...
setwd("/Users/natasha/Documents/Research/relman-lab/docker_access/sea_otter/vPaper/gingival_community_profiles")
getwd()
```
...
Load phyloseq object (full otter dataset)

```{r load_physeq}
physeq <- readRDS("/Users/natasha/Documents/Research/relman-lab/docker_access/sea_otter/vPaper/physeq_dec18.rds")
```
...
plot(p)
ggsave('rda_gingival.pdf', plot = p)
```
...
plot(p)            
ggsave('so_richness_gingivalCP.pdf', plot = p)
```
...
plot(p)
ggsave('deseq2_gingivalCPs.pdf', plot = p)
```
...
write.csv(otu_table(cp1_ps), file = "cp1_otus.csv")
write.csv(otu_table(cp2_ps), file = "cp2_otus.csv")
write.csv(otu_table(cp3_ps), file = "cp3_otus.csv")
```
...
plot(p)
ggsave('year_mo_ellipses.pdf', plot = p)
```
...
p    
ggsave('cp_mo_barplot.pdf', plot = p)
```